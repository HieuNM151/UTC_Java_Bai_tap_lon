...
public class StatisticsService {
    ...
    public Map<String, BigDecimal> getRevenueByDay(LocalDate date) {
        String sql = "SELECT TRUNC(NgayTao) AS Ngay, SUM(TongTien) AS DoanhThu " +
                     "FROM HoaDon " +
                     "WHERE TRUNC(NgayTao) = ? AND TinhTrang = 1 " +
    ...
    public Map<String, BigDecimal> getRevenueByMonth(YearMonth yearMonth) {
        String sql = "SELECT SUM(TongTien) AS DoanhThu " +
                     "FROM HoaDon " +
                     "WHERE EXTRACT(YEAR FROM NgayTao) = ? AND EXTRACT(MONTH FROM NgayTao) = ? AND TinhTrang = 1";
        Map<String, BigDecimal> result = new HashMap<>();
        try (PreparedStatement ps = getConnection().prepareStatement(sql)) {
    ...
    public Map<String, BigDecimal> getRevenueByYear(int year) {
        String sql = "SELECT SUM(TongTien) AS DoanhThu " +
                     "FROM HoaDon " +
                     "WHERE EXTRACT(YEAR FROM NgayTao) = ? AND TinhTrang = 1";
        Map<String, BigDecimal> result = new HashMap<>();
        try (PreparedStatement ps = getConnection().prepareStatement(sql)) {
            ps.setInt(1, year);
            try (ResultSet rs = ps.executeQuery()) {
    ...
    public Map<String, BigDecimal> getRevenueByDateRange(LocalDate startDate, LocalDate endDate) {
        String sql = "SELECT SUM(TongTien) AS DoanhThu " +
                     "FROM HoaDon " +
                     "WHERE TRUNC(NgayTao) BETWEEN ? AND ? AND TinhTrang = 1";
        Map<String, BigDecimal> result = new HashMap<>();
        try (PreparedStatement ps = getConnection().prepareStatement(sql)) {
    ...
    public BigDecimal getProfitMarginByDateRange(LocalDate startDate, LocalDate endDate) {
        String sql = "SELECT " +
                     ...
                     "WHERE TRUNC(h.NgayTao) BETWEEN ? AND ? AND h.TinhTrang = 1";
        try (PreparedStatement ps = getConnection().prepareStatement(sql)) {
            ps.setDate(1, java.sql.Date.valueOf(startDate));
            ps.setDate(2, java.sql.Date.valueOf(endDate));
            try (ResultSet rs = ps.executeQuery()) {
    ...
    public List<Map<String, Object>> getTopSellingProducts(LocalDate startDate, LocalDate endDate, int limit) {
        String sql = "SELECT ct.Id, ct.Ten, ct.Ma, ct.GiaNhap, ct.GiaBan, " +
    ...
    public List<Map<String, Object>> getProductsInStock(int limit) {
        String sql = "SELECT Id, Ten, Ma, SoLuongTon, GiaNhap, GiaBan " +
                     "FROM ChitietSP " +
                     "WHERE SoLuongTon > 0 " +
    ...
    private Connection getConnection() throws SQLException {
        return com.quanlybanhang.config.DatabaseConfig.getInstance().getConnection();
    }
}
